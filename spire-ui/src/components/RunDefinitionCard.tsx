...
export default function RunDefinitionCard({ run }: { run: RunView }) {
  ...
  // Which key paid, in the operator's words. Every harness credential here is an API key, billed per
  // token — a subscription is not a credential this deployment holds, so the card never implies one.
  const billing = run.credentialLabel ? `API key ${run.credentialLabel}${run.credentialType ? ` · ${run.credentialType}, billed per token` : ''}` : null;
