...
| Work in progress | While a call is in flight the primary control's label becomes the present participle ("Approving…", "Registering…", "Saving…"), and a `role="status"` line says what is happening; a read sets `aria-busy` and announces itself. Static words, never an unbounded spinner. A disabled control with an unchanged label reads as a click that did nothing | [Approvals](../src/components/work-items/Approvals.tsx) |
| Text the system composed, not a record a person keeps | Say it was composed, and link where it came from, rather than rendering it as another ticket: an operator sent to edit that ticket would be editing something the decision no longer reads. Pair it with an explicit action that composes again | [WorkItemSteps](../src/components/work-items/WorkItemSteps.tsx), [DecisionEvidence](../src/components/work-items/DecisionEvidence.tsx) |
| A pool whose members can be exhausted | One row per member with its state as a chip, and only the action that state allows: a rate limit lifts by itself, a rejection needs a person. A write-only secret is entered in a side panel and never read back | [SettingsHarnessCredentials](../src/components/SettingsHarnessCredentials.tsx) |
| Coordinates a deployment can actually run | Offer them as selects built from what is configured (agent images) and enabled (catalogue models); keep an unusable choice visible but disabled with the reason on it, and keep a stored value that is no longer offered as a labelled option rather than dropping it | [BuildStep](../src/components/repositories/factory/BuildStep.tsx), [WorkItemPreparation](../src/components/work-items/WorkItemPreparation.tsx) |
| Later-than-the-call work | A control that only requests work (a scan) says so and names when the work happens; it never claims the work finished | [SourceStep](../src/components/repositories/factory/SourceStep.tsx) |
...
| Where a work item stands | `JourneyStrip` and `JourneyLegend`: done phases solid, the current phase by how it stands (waiting, blocked, running, stopped, failed), later phases by who decides them. Rules live in `workJourney.ts` so the list and the detail draw the same answer | [JourneyStrip](../src/components/work-items/JourneyStrip.tsx) |
| A list a person triages | Filter chips that carry their counts across every page, an `attn` band that counts what needs a person, the reason as a sentence and the one next action in the row. The list re-reads on a timer while visible, keeps its rows during a re-read, and says when it was last updated or that it is stale | [WorkItems](../src/components/work-items/WorkItems.tsx) |
| A decision someone must answer | `DecisionPanel` in a wide `SidePanel` beside the list it came from, addressed by `?decide=<id>` so a link opens it. It shows what the decision binds, what answering starts, how long it stays open and the other channels before offering Approve and Reject | [DecisionPanel](../src/components/work-items/DecisionPanel.tsx) |
| One item as its journey | `WorkItemSteps`: the `factory-step` list with three states a run adds (failed, stopped, later). Done steps carry proof from the current attempt only, the current step carries the reason and the actions, later steps say who decides them, and a finished journey keeps its actions below the list. A re-read keeps the item on screen so an open panel survives it | [WorkItemSteps](../src/components/work-items/WorkItemSteps.tsx) |
