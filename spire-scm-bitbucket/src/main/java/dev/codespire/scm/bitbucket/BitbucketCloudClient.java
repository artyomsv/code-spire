...
public class BitbucketCloudClient {
    ...
    private String send(String method, String path, String jsonBody, boolean originRequired) {
        ...
        for (int hop = 0; hop <= MAX_REDIRECTS; hop++) {
            ...
            if (status / 100 == 3) {
                ...
                if (originRequired && !sameOrigin(target)) throw new BitbucketApiException(status, method, path, "identity redirect left configured origin");
    ...
    private URI redirectTarget(URI current, String location, int status, String method, String path) {
        try {
            ...
            throw new BitbucketApiException(status, method, path, "unparseable redirect target refused");
        }
    }
