...
All creation/edit fieldsets retain pending-save lock-out, including Cancel. Profile fields explain
...
- **27 distinct production mutations**, each one selected assertion failure, exact scratch-byte
...
The dev UI was rebuilt and recreated with --no-deps; it answers HTTP 200 on port 39285. The two
deployed setup-screen source hashes match the tested worktree. No migration, run worker or local
Java service test tier was needed for these frontend changes. PR #153 remains draft for operator review.
