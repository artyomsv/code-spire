...
- [security/M-1 (cap half)] `POST /api/runs` applied none of ADR-025's gates while V42 counts run spend in the window — `SpendGate.decide()` before the queued row, 429; `RunResourceTest` — round 1
...
- [arch/merge-94] the qa-R3 2 fix named `DockerRunRuntime` in `RunAckBudget`, which the neutrality scan refuses in a core module — unseen because the round-3 commit never got a CI run (the PR was already conflicting with master); the window is now `RunRuntime.drainWindow()`, read through the injected arm, with a seam test — master merge
