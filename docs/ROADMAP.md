...
| **P4** | Learned memory + per-author analytics | 🚧 **corpus + analytics delivered** | `review_finding` (V36) is the durable per-finding record the roadmap assumed already existed — it did not, and findings were being discarded continuously (inline on Kafka under short retention, plus one overwritten `review_status` column). No backfill, so the corpus accrues from here. Analytics (FR-11) ships with it because reading a projection back is the only way to tell a correct one from a wrong one. Learned memory (FR-10) follows: operator-approved preferences that filter visibly rather than steering the prompt invisibly. ADR-027. |
