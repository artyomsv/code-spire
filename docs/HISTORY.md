...
- **Code-scanning backlog cleared (2026-08-05):** the Security tab's 120 open alerts are closed at
  ...
  **The Trivy half split by who can fix it.** The 39 OS-package alerts (libexpat, p11-kit) are
  ...
  same scan job, clean base. (**Both halves of that sentence were later measured wrong** — the
  upgrade layer cached and so ran once, and `spire-ui`'s base was clean only until its next scan.
  See the 2026-09-04 entry.) The 24 Java-dependency alerts were mostly closed by the **platform**,
...
- **Per-repository prompts and conversation-derived findings delivered (ROADMAP E16/E17, 2026-08-24):**
  ...
  - **Conversation-derived findings.** A `/finding` command (`ConfirmFinding` → `ConversationFindingRaised`
    ...
    **(Closed in M2 and the entry is deleted. The product decision went against the commands:
    every SCM-originated trigger is refused in observe mode — `/command`, author reply, and the
    archived notice, the latter two found by review rather than by the plan. The "an explicit command is an
