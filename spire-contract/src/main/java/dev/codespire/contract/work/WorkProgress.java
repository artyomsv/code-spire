...
public record WorkProgress(UUID attemptId, String attemptPhase, String attemptState, boolean reserved,
    ...
    public WorkProgress reserve(boolean value) { return new WorkProgress(attemptId,attemptPhase,attemptState,value,startedAt,runs,steps,wallSeconds,costMillicents,calls,usageUnknown); }
    public WorkProgress unknownUsage() { return new WorkProgress(attemptId,attemptPhase,attemptState,reserved,startedAt,runs,steps,wallSeconds,costMillicents,calls,true); }
    public WorkProgress start(UUID id,String phase,Instant now) {
        boolean build="build".equals(phase);
        return new WorkProgress(id,phase,"started",true,now,runs+(build?1:0),steps+(build?1:0),wallSeconds,costMillicents,calls,usageUnknown);
    }
    public WorkProgress finish(long wall,long cost,long callCount) {
        ...
        return new WorkProgress(attemptId,attemptPhase,"completed",false,startedAt,runs,steps,
                Math.addExact(wallSeconds,wall),Math.addExact(costMillicents,cost),Math.addExact(calls,callCount),usageUnknown);
    }
