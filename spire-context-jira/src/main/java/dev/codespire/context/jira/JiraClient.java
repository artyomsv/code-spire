...
/**
 * Thin read-only HTTP layer over the Jira REST API (v2 — its {@code description}
 * comes back as a plain string, unlike v3's Atlassian Document Format, so no ADF
 * walker is needed and Data Center is covered by the same paths).
 *
 * <p>As in the SCM adapters, redirects are followed MANUALLY with host pinning —
 * the bot's Authorization header is only ever sent to the configured API host,
 * never to a cross-host redirect target (SSRF guard). The credential is never
 * logged.
 */
public class JiraClient {
    ...
    private String send(String method, String path) {
        ...
        for (int hop = 0; hop <= MAX_REDIRECTS; hop++) {
            ...
            if (status / 100 == 3) {
                String location = response.headers().firstValue("Location")
                        .orElseThrow(() -> new JiraApiException(status, method, path));
                target = target.resolve(location);
                requireSafeRedirectTarget(target, status, method, path);
                continue;
            }
            if (status / 100 != 2) {
                throw new JiraApiException(status, method, path, bodySnippet(response.body()));
            }
            ...
            if (!looksLikeJson(contentType, body)) {
                throw new JiraApiException(status, method, path,
        ...
        throw new JiraApiException(310, method, path); // too many redirects
    }
    ...
    private void requireSafeRedirectTarget(URI target, int status, String method, String path) {
        ...
        if (host == null) {
            throw new JiraApiException(status, method, path, "redirect without a host refused");
        }
        ...
        try {
            ...
            throw new UncheckedIOException("Jira API " + method + " " + path
    ...
    private HttpResponse<String> execute(String method, String path, URI target) {
        ...
        try {
            ...
            throw new UncheckedIOException("Jira API " + method + " " + path + " I/O failure", e);
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new IllegalStateException("Interrupted calling Jira API", e);
        }
    }
    ...
    private JsonNode parse(String body) {
        try {
            ...
            throw new UncheckedIOException("Unparseable Jira API response", e);
        }
    }
}
