...
final class PinnedHttpTransport {
    ...
    PinnedHttpTransport(PinnedJsonConfig config, ObjectMapper mapper, HttpFailures failures) {
    ...
    private String send(String method, String path, boolean requireJsonShape, boolean originRequired) {
        return response(method, path, requireJsonShape, originRequired, null).body();
    }

    PinnedJsonResponse identityResponse(String path) {
        HttpResponse<String> response = response("GET", path, true, true, null);
        return new PinnedJsonResponse(parse(response.body()), response.headers().map());
    }

    JsonNode write(String method, String path, String json) {
    ...
    private HttpResponse<String> response(String method, String path, boolean requireJsonShape,
        ...
        for (int hop = 0; hop <= MAX_REDIRECTS; hop++) {
            HttpResponse<String> response = execute(method, path, target, maxBytes, json);
            int status = response.statusCode();
            if (status / 100 == 3) {
                // A write is not replayed onto a redirect target, even on the same origin.
                if (!"GET".equals(method)) throw failures.create(status, method, path, "write redirect refused");
            ...
            if (!requireJsonShape) {
                return response; // body is null when the bounded subscriber aborted past MAX_RAW_BYTES
            }
    ...
    private HttpResponse<String> execute(String method, String path, URI target, int maxBytes, String json) {
