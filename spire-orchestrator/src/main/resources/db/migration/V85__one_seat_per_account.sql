...
-- the file carries tokens.account_id. Two seats on one account would share one subscription's limits
-- while looking like two, and keep an older, expiring copy in use, so an enabled seat's account is
-- unique per harness. Signing in to the same account again
