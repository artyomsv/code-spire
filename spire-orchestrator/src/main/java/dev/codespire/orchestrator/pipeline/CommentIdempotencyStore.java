...
/**
 * Insert-before-post idempotency for comment posting (ADR-013).
 *
 * Semantics (recovery-aware — review finding H1):
 ...
 *   retry re-posts instead of silently losing the comment. The narrow window
 *   "posted but crashed before markPosted" can then produce one duplicate;
 *   at-least-once posting is preferred over silent loss.
 */
@ApplicationScoped
public class CommentIdempotencyStore {
    ...
    public Claim claim(String reviewId, String commit, String anchorKey) {
        // Reclaim NULL-comment_id rows; leave posted rows untouched.
        String upsert = """
