...
public class DiffWorker {
    ...
    public void fetchDiff(FetchDiff command) {
        registry.remember(command.reviewId(), command.repo());
        try {
            Diff diff = diffSource.fetchDiff(command.repo(), command.prId(), command.commit());
            results.send(new DiffFetched(
            ...
            if (e.isNotFound()) {
                ...
                timeline.record("command", "abandoned:FetchDiff", command.reviewId(),
                        "diff 404 — commit force-pushed away");
                return;
            }
            fail(command, e);
    ...
    private void fail(FetchDiff command, RuntimeException e) {
        ...
        results.send(new ReviewFailed(command.reviewId(), command.commit(), "fetch-diff",
                        e.getMessage(), retryable, 1))
                .whenComplete((v, t) -> {
                    if (t != null) {
                        LOG.warnf(t, "Failed to emit ReviewFailed for %s", command.reviewId());
                    }
                });
    }
