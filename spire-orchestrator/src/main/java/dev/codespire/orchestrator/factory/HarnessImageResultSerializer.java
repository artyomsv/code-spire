...
/**
 * The orchestrator never publishes a {@link HarnessImageResult} — the worker does — but the
 * dead-letter queue on {@code harness-image-results-in} re-serializes a record whose processing
 * failed, and SmallRye resolves that serializer by rewriting the configured deserializer's class name
 * ({@code HarnessImageResultDeserializer} → {@code HarnessImageResultSerializer}). Without this class
 ...
 */
public class HarnessImageResultSerializer implements Serializer<HarnessImageResult> {

    private final ObjectWriter writer = resolveMapper().writerFor(HarnessImageResult.class);
    ...
    @Override
    public byte[] serialize(String topic, HarnessImageResult result) {
        if (result == null) {
            return null;
        }
        try {
